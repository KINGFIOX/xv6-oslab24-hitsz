...
int fetchaddr(uint64 addr, uint64 *ip) {
  ...
  if (copyin(p->su_space, (char *)ip, addr, sizeof(*ip)) != 0) return -1;
  return 0;
}
...
int fetchstr(uint64 addr, char *buf, int max) {
  struct proc *p = myproc();
  int err = copyinstr(p->su_space, buf, addr, max);
  if (err < 0) return err;
  return strlen(buf);
}
...
static uint64 (*syscalls[])(void) = {
    ...
    [SYS_link] sys_link,   [SYS_mkdir] sys_mkdir,   [SYS_close] sys_close, [SYS_checkvm] sys_checkvm,
};
